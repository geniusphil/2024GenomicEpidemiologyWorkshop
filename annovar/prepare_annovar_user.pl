...
our $REVISION = '$Revision: 2abe1a1530386dbdd7ac3f29b59547ff26ba2214 $';
our $DATE =	'$Date: 2024-02-19 10:15:59 -0500 (Mon, 19 Feb 2024) $';  
...
sub prepareCosmic {
	...
	my $headerline = $_;
	my ($mutid, $filetype);
	my ($mutid_index, $id_tumor_index, $primary_site_index, $phenotype_index, $sample_index);
	
	if ($field[16] eq 'Mutation ID' or $field[16] eq 'GENOMIC_MUTATION_ID') {	#'Mutation ID' is for v89 and before, 'GENOMIC_MUTATION_ID' is for v90
		$mutid_index = 16;
		$mutid = $field[$mutid_index];
		$filetype = 'coding';
	} elsif ($field[17] eq 'GENOMIC_MUTATION_ID') {	#'GENOMIC_MUTATION_ID' is for v91, v92, etc in the 18th column (one extra column called 'MUTATION_ID' is added before this column), the ID_NCV is no longer in the noncoding file either
		$mutid_index = 17;
		$mutid = $field[$mutid_index];
		$filetype = 'coding';
	} elsif ($field[11] eq 'ID_NCV' or $field[11] eq 'GENOMIC_MUTATION_ID') {	#'ID_NCV' is for v89 and before, 'GENOMIC_MUTATION_ID' is for v90
		$mutid_index = 11;
		$mutid = $field[$mutid_index];
		$filetype = 'noncoding';
	} elsif ($headerline =~ m/\bGENOMIC_MUTATION_ID\b/) {
		for my $i (0 .. @field-1) {
			if ($field[$i] eq 'GENOMIC_MUTATION_ID') {
				$mutid_index = $i;
			} elsif ($field[$i] eq 'ID_tumour') {
				$id_tumor_index = $i;
			} elsif ($field[$i] eq 'Primary site') {
				$primary_site_index = $i;
			} elsif ($field[$i] eq 'COSMIC_PHENOTYPE_ID') {
				$phenotype_index = $i;
			} elsif ($field[$i] eq 'COSMIC_SAMPLE_ID') {
				$sample_index = $i;
			}
		}
		$filetype = 'unknown';
	} else {
		die "Error: COSMIC MutantExport format error: column 17 or 18 or 12 should be 'Mutation ID' or 'GENOMIC_MUTATAION_ID' or 'ID_NCV', or at least 'GENOMIC_MUTATAION_ID' should be present in the header line\n";
	}
	defined $mutid_index or die "Error: Unable to locate Mutation ID column from the header line of the input file $dbfile\n";
	$filetype eq 'unknown' and print STDERR "NOTICE: unknown file type with mutid_index=$mutid_index, phenotype_index=$phenotype_index, sample_index=$sample_index\n";	

	while (<DB>) {
		my ($id_tumor, $primary_site);
		@field = split (/\t/, $_);
		
		if ($filetype eq 'coding') {
			$mutid = $field[$mutid_index];	#it could be 16 or 17
			$id_tumor = $field[6];
			$primary_site = $field[7];
		} elsif ($filetype eq 'noncoding') {
			$mutid = $field[$mutid_index];						#it is 11 but may change to a different number in the future
			$id_tumor = $field[2];
			$primary_site = $field[3];
		} elsif ($filetype eq 'unknown') {	#like cosmic99
			$mutid = $field[$mutid_index];
			if (defined $id_tumor_index) {
				$id_tumor = $field[$id_tumor_index];
			} elsif (defined $sample_index) {
				$id_tumor = $field[$sample_index];
			}
			if (defined $primary_site_index) {
				$primary_site = $field[$primary_site_index];
			} elsif (defined $phenotype_index) {
				$primary_site = $field[$phenotype_index];
			}
		}
		...
		if ($id_tumor) {	#does not exist in cosmic99 but we are using sample_index and phenotype_index instead
	...
	print STDERR "NOTICE: Finished reading ", scalar (keys %cosmicid), " COSMIC records in DB file $dbfile\n";
	...
	for my $key (keys %cosmicid) {
		if ($cosmic{$key}) {
			...
			$occurence =~ s/^,//;
			print "$occurence\n";
		} else {		#for future cosmic version, if sample/phenotype information is not available, then we only print out cosmic genomic_mutation_id
			my @newcosmicid = split (/,/, $cosmicid{$key});
			shift @newcosmicid;
			my %newcosmicid = map {$_, 1} @newcosmicid;
			print $key, "\t", "ID=", join (",", keys %newcosmicid), "\n";
		}
	}		
}
...
=head1 SYNOPSIS
 ...
 Example: # For preparing COSMIC database for annotation in ANNOVAR
          ...
          prepare_annovar_user.pl -dbtype cosmic CosmicNCV.tsv -vcf CosmicNonCodingVariants.vcf.gz > hg38_cosmic90_noncoding.txt
          prepare_annovar_user.pl -dbtype cosmic CosmicMutantExport.tsv -vcf CosmicCodingMuts.vcf.gz >> hg38_cosmic90_coding.txt
 ...
 Version: $Date: 2024-02-19 10:15:59 -0500 (Mon, 19 Feb 2024) $
